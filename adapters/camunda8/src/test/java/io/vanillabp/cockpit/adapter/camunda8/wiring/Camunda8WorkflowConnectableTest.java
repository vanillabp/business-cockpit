...
class Camunda8WorkflowConnectableTest {
    ...
    @BeforeEach
    void setUp() {
        connectable = new Camunda8WorkflowConnectable(
                "my-module",
                "tenant-1",
                process,
                "v1.0");
    ...
    @Test
    void getElementId_delegatesToProcessId() {
        when(process.getId()).thenReturn("order-process");
        assertThat(connectable.getElementId()).isEqualTo("order-process");
    }
    ...
    @Test
    void getTaskDefinition_delegatesToProcessId() {
        when(process.getId()).thenReturn("order-process");
        assertThat(connectable.getTaskDefinition()).isEqualTo("order-process");
    }
