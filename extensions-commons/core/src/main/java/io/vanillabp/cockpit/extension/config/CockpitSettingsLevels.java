 ...
 * {@link io.vanillabp.integration.extension.spi.settings.SettingsResolution} knows it for every
 ...
 * adapter, and the cockpit will want that the day a value has to differ between two clusters of one
 * BPMS type; until then nothing binds those keys, and a position nothing binds is not a position an
...
public final class CockpitSettingsLevels {
  ...
  /**
   * The application. It says nothing about the keys which exist at several levels: what the whole
   * application configures - the transport, the templates, the two switches - has one place and is
   * read there, not resolved per workflow.
   */
  ...
  /**
   * One workflow. A user task says nothing which is resolved this way - what it carries is a
   * segment of the template path, and a path is assembled from the levels rather than overridden
   * by the most specific one - so the walk ends here.
   */
